...
function gitPushInternal(htmlContent, cb) {
  ...
  const steps = [];
  // Clean up any stale lock file before doing anything else. With proper
  // serialization (see gitPush below) this function only ever runs one
  // at a time, so any lock file that still exists at this point can only
  // be a leftover from an earlier crashed/killed process (e.g. a
  // container restart mid-operation, or an overlapping save from before
  // this serialization existed) — never a legitimately still-running
  // operation of our own. Safe to remove unconditionally.
  steps.push(() => {
  ...
  // Sync to whatever is ACTUALLY on GitHub right now before committing.
  // Render can spin up a fresh container from an older commit and then
  // auto-redeploy again in response to our own auto-push — if a second
  // save lands on one of these slightly-stale containers, its local git
  // history no longer matches the real current remote, and a plain push
  // gets rejected as non-fast-forward ("fetch first"). Fetching and hard
  // resetting to the real remote HEAD first — then re-writing our saved
  // content back on top of that — guarantees every commit builds on the
  // actual latest state, so this can't happen no matter how many
  // containers or redeploys are in flight.
  steps.push(() => run('git', ['fetch', 'origin', targetBranch]).catch(() => null));
  steps.push(() => run('git', ['reset', '--hard', 'origin/' + targetBranch]).catch(() => null));
  // The reset above overwrites index.html on disk with whatever GitHub
  // currently has — re-apply our actual saved content on top of that.
  ...
  steps.push(() => run('git', ['commit', '-m', 'Admin save: ' + new Date().toISOString()])
    .catch(err => {
      // Git phrases this two different ways depending on whether other
      // untracked files (backups/, node_modules/) happen to be sitting in
      // the working directory: "nothing to commit, working tree clean" if
      // not, or "nothing added to commit but untracked files present" if
      // so. Both mean the same thing for us — match both.
  ...
  // whether we're on a branch or detached — and since we just reset onto
  // the real remote HEAD above, this push is always a clean fast-forward.
...
// Serialize all git operations through a single queue. Without this, two
// /__save requests arriving close together (a double-click, two browser
// tabs, a retried request) could run their git commands concurrently
// against the same working directory — which is exactly what produced a
// real "could not lock config file .git/config: File exists" error: two
// overlapping `git config` calls collided. Every gitPush() call now
// chains onto this single promise instead of running immediately, so
// only one is ever touching the repo at a time; everything else just
// waits its turn.
let gitQueue = Promise.resolve();
function gitPush(htmlContent, cb) {
  gitQueue = gitQueue
    .then(() => new Promise(resolve => gitPushInternal(htmlContent, err => { cb(err); resolve(); })))
    .catch(() => {}); // gitPushInternal already reports its own error via cb; never let the queue itself die
}
